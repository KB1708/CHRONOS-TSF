...
You should see a message indicating the server is running on http://120.0.0.1:8000. Leave this terminal running.

2. Start the Frontend UI
Open a second, new terminal, navigate to the same project root, and activate the virtual environment again (venv\Scripts\activate). Then, run the following command to start the Streamlit frontend:
...
How to Use the App
Select a Dataset: Use the sidebar to either upload your own CSV file or choose one of the pre-loaded sample datasets.

Configure Columns: In the main panel, use the dropdowns to select the correct "Date" and "Value" columns from your data.

Set Forecast Horizon: Use the slider in the sidebar to choose how many months into the future you want to forecast.

Generate Forecast: Click the "Generate Forecast" button to run the analysis. The results, including metrics and graphs, will be displayed on the main page.